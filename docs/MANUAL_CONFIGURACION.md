...
Módulo genérico del **sujeto**, no atado a un rubro. Cada sede es dueña de su cuaderno. Los **tipos de registro** (en veterinaria: Consulta, Control, Vacuna, Estudio) y sus **campos** (peso, temperatura, diagnóstico/hallazgos, tratamiento, producto/lote, próxima fecha, resultados) se configuran por red en **Configuración → Tipos de registro**. El admin de red habilita el módulo, las sedes y los tipos; **no** abre el contenido. La operación (ficha con timeline, acciones rápidas, finalizar con campos obligatorios, adenda, compartir, imprimir) queda en admin de sede y operador. Adjuntos privados (PDF/foto con miniatura autorizada) viven en el registro. Lo finalizado es inmutable (adendas agrupadas). Hay PDF A4 de un registro final y de la ficha completa (carátula con último peso y próximos; solo finales de esa sede; se audita cada descarga). **Compartir con caso** es manual y no envía adjuntos ni el historial completo. El operador puede abrir la ficha en solo lectura; no crea sujetos.

## 11c. Catálogo y orden de venta (por clínica)

Cada clínica administra su **catálogo liviano** (`Operativa → Catálogo de venta`) y los **datos comerciales** en la ficha de la sede (moneda ISO, prefijo, emisor). El operador usa ítems activos al armar la orden; no edita precios maestros. La orden nace **solo** de un registro clínico **final** de esa sede, queda persistida (borrador → emitida inmutable → exportada), con PDF y CSV genérico UTF-8 para el ERP. SURC no calcula comprobantes fiscales ni se conecta por API al facturador. La seña del caso no se convierte en línea. La anfitriona de una agenda abierta no ve la orden de la clínica de origen. Permisos: `sales.catalog.manage` (admin de clínica), `sales.orders.manage` / `sales.orders.export` (admin de clínica y operador). El dueño de plataforma y el admin de red no leen órdenes clínicas.
...
| organization_admin | Su sede | `cases.manage`, `payments.confirm`, `history.view/manage/finalize/share/print`, `sales.catalog.manage`, `sales.orders.manage/export` — listados y formularios enfocados en su sede; opera historial, catálogo y órdenes |
| operator | Operar casos de su sede (si tiene sede asignada) | `cases.operate`, `payments.confirm`, `history.view/manage/finalize/share/print`, `sales.orders.manage/export` — ve listados de Agenda/Casos y workspace; **no** crea ni edita agendas/casos/sujetos. **Sí** confirma señas. Con historial activo, abre la ficha del sujeto en solo lectura, opera el cuaderno y emite órdenes. **No** administra el catálogo |
