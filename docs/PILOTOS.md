...
- Criterios: flags red+sede; ficha del sujeto por sede (timeline, acciones rápidas, adjuntos autorizados); admin de red configura sin ver contenido; admin de clínica/operador operan el cuaderno; operador ve y carga historial sin editar datos base; campos del tipo y adjuntos privados; borrador incompleto → final con required → adenda (adenda no es fila principal); PDF de registro final y ficha completa (carátula + próximos) auditados; próximos de mi clínica solo finales de la sede; compartir solo finales con destino visible (sin adjuntos ni cuaderno); incorporar una vez desde caso cerrado; otra sede/especialista/dueño de plataforma/admin de red sin el cuaderno; un usuario de sede no puede dar de alta sujetos ni casos en otra clínica.
...
- Orden de venta: catálogo aislado por clínica; orden solo desde registro final de la sede dueña del historial (aunque se atienda en agenda abierta); emitida inmutable; CSV/PDF sin diagnóstico ni adjuntos; anfitriona/especialista/admin de red/dueño de plataforma no abren la orden.
